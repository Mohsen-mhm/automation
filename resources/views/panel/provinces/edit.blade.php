    ...
    <div class="mb-6">
        ...
        <div class="flex items-center space-x-3 rtl:space-x-reverse">
            <label class="inline-flex items-center">
                <input type="radio" name="active" value="1" {{ $province->active ? 'checked' : '' }}
                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
                <span class="mr-2 text-sm font-medium text-gray-900">فعال</span>
            </label>
            <label class="inline-flex items-center">
                <input type="radio" name="active" value="0" {{ !$province->active ? 'checked' : '' }}
                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
    ...
    <div class="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-xl">
        ...
        <div class="space-y-2 text-sm">
            <div class="flex items-center justify-between">
                <span class="text-gray-600">کل شهرها:</span>
                <span class="font-medium text-gray-900">{{ $province->cities_count }}</span>
            </div>
            <div class="flex items-center justify-between">
                <span class="text-gray-600">شهرهای فعال:</span>
                <span class="font-medium text-gray-900">{{ $province->active_cities_count }}</span>
            </div>
            <div class="flex items-center justify-between">
                <span class="text-gray-600">تاریخ ایجاد:</span>
                <span class="font-medium text-gray-900">{{ $province->created_at->format('Y/m/d H:i') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
        ...
        <div class="space-y-2 text-sm">
            <div class="flex items-center justify-between">
                ...
                <span class="font-medium text-blue-900" id="previewName">{{ $province->name }}</span>
            </div>
            <div class="flex items-center justify-between">
                <span class="text-blue-700">ترتیب نمایش:</span>
                <span class="font-medium text-blue-900" id="previewSort">{{ $province->sort_order }}</span>
            </div>
            <div class="flex items-center justify-between">
                <span class="text-blue-700">وضعیت:</span>
                <span class="font-medium text-blue-900"
                      id="previewStatus">{{ $province->active ? 'فعال' : 'غیرفعال' }}</span>
            </div>
        </div>
    </div>
